// استدعاء الفايربيز بشكل مباشر مضافاً إليه دالة الـ runTransaction للتوليد التلقائي
import { initializeApp } from "https://www.gstatic.com/firebasejs/9.22.1/firebase-app.js";
import { getFirestore, collection, addDoc, getDocs, doc, runTransaction, updateDoc, query, where } from "https://www.gstatic.com/firebasejs/9.22.1/firebase-firestore.js";
...
// ==========================================
// 2. منطق التحكم في النافذة المنبثقة (Modal) والتوليد التلقائي
// ==========================================
...
// حفظ مريض جديد بالتوليد التلقائي والترتيب التصاعدي المضمون
if (addPatientForm) {
    addPatientForm.addEventListener("submit", async (e) => {
        ...
        // مرجع مستند العدادات لمنع التكرار نهائياً
        const counterRef = doc(db, "counters", "patients_counter");

        try {
            // تشغيل الـ Transaction لقراءة وزيادة العداد في خطوة واحدة محمية
            const nextId = await runTransaction(db, async (transaction) => {
                const counterDoc = await transaction.get(counterRef);
                
                let currentId = 0;
                if (counterDoc.exists()) {
                    currentId = counterDoc.data().currentValue || 0;
                }
                
                let newId = currentId + 1;
                transaction.set(counterRef, { currentValue: newId });
                return newId;
            });

            const finalPatientId = String(nextId); // تحويل رقم العميل المسلسل لنص

            // حفظ المريض بالكود الرقمي الجديد
            await addDoc(collection(db, "patients"), {
                customId: finalPatientId, 
                name: name,
                phone: phone,
                dob: dob,
                diagnosis: diagnosis,
            ...
            alert(`تم حفظ المريض بنجاح! كود المريض التلقائي هو: [ ${finalPatientId} ]`);
            addPatientForm.reset();
            addPatientModal.style.display = "none";
            
            // تحويل فوري لصفحة المريض الجديد
            window.location.href = `patient.html?id=${finalPatientId}`;
            
        } catch (error) {
            console.error("خطأ أثناء توليد الكود أو الحفظ:", error);
...
// ==========================================
// 4. تشغيل كاميرا الـ QR Code من الداشبورد (نسخة جيت هاب والموبايل المضمونة)
// ==========================================
...
if (scanQRBtn) {
    scanQRBtn.addEventListener("click", async () => {
        ...
        // 1. لو الكاميرا شغالة بالفعل وضغطنا تاني، نقفلها ونعمل Toggle
